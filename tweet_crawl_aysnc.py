...
#
#
# async def extract_tweet_id(tweet_url):
#     try:
...
#                     return data, "done"
#                 else:
#                     await token_mgr.ensure_valid_tokens(tweet_url, resp.status)
#                     print(f"⚠️ Non-200 ({resp.status}) for {tweet_url}, retrying...")
#                     await asyncio.sleep(2 ** attempt)
#
#         except aiohttp.ClientConnectionError as e:
#             print(f"⚠️ Connection error fetching {tweet_url}: {e}")
#             await asyncio.sleep(2 ** attempt)
#         except aiohttp.ClientError as e:
#             print(f"⚠️ Client error fetching {tweet_url}: {e}")
#             await asyncio.sleep(2 ** attempt)
#         except Exception as e:
#             print(f"⚠️ Unexpected error fetching {tweet_url}: {type(e).__name__} - {e}")
#             await asyncio.sleep(2 ** attempt)
#
#     return None, "error"
...
async def extract_tweet_id(tweet_url: str) -> str:
    """Extract tweet ID safely from a Twitter/X URL."""
    try:
...
async def fetch_tweet_data(session: aiohttp.ClientSession, token_mgr: TokenManager, tweet_url: str, retries: int = 3):
    """
    Fetch tweet JSON data from Twitter/X GraphQL API.
    Includes retry, rate-limit handling, and token refresh logic.
    """
    tweet_id = await extract_tweet_id(tweet_url)
    if not tweet_id:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            print(f"🌐 Connection error on {tweet_url}: {e}")
            await asyncio.sleep(2 ** attempt)

        except aiohttp.ClientError as e:
            print(f"⚠️ Client error on {tweet_url}: {e}")
            await asyncio.sleep(2 ** attempt)

        except asyncio.TimeoutError:
            print(f"⏱️ Timeout fetching {tweet_url}, retry {attempt}/{retries}")
            await asyncio.sleep(2 ** attempt)
